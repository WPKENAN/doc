...
\cnabstract{近些年来，随着社交网络服务蓬勃发展，互联网世界快速进化，社交网络在全球拥有着大量用户，并且使用人数在日益增加，已然成为影响巨大的信息平台。掌握其中有利的信息例如社交网络中用户的行为特征、所具有以及发布的信息所具有的传播规律，甚至内容的定位等，不仅能帮助企业根据客户所需制定出更好更完善及用户友好型的产品，提供更有力的服务，还可以进行更加有效的网络媒体营销。另外，还可为政府及相关部门在舆论控制方面提供有利理论依据和执行的条件，从而优化社会风气。
	所以，在本科生毕业设计时，我选择了基于社交网络的数据采集以及分析。本文中提出并实现了一种，基于新浪微博程序接口和网络数据流相互结合的方式进行数据的采集文献，利用cookie对新浪微博网页端进行了模拟登陆，实现了数据抓取采集。并且通过监测信息交互时传输的数据包。进行动态抓取页面信息。利用python语言网络爬虫和json等数据库函数，解析HTML XML，获得用户的数据信息。然后通过基于图的聚类的社交网络算法以及PageRank算法，进行用户节点数据的建模。收集用户的偏好以及计算其节点的重要性，对微博中用户与用户之间的社交网络关系进行分析，通过多种不同算法对节点的重要性进行了分析，对比多种算法之间的精准度， 对其间的影响力评价模型进行了实现。
	本文中所讨论的社交网络用户数据信息的获取，主要问题是在于解决了微博近些年来日渐难以爬取大量数据的问题并通过数据挖掘对对用户行为进行分析研究，得到了具体的用户影响力公式，并根据实际进行了验证与误差分析。
	.}

\cnkeyword{社交网络, 数据采集, 数据处理与分析,  数据挖掘, 图聚类}
...
\enabstract{In recent years, with the rapid development of social networking services and the rapid evolution of the Internet world, social networks have a large number of users in the world, and the number of users is increasing. It has become a huge information platform. Mastering the beneficial information such as the behavioral characteristics of the users in the social network, the propagation rules of the information they have and the information they publish, and even the positioning of the content can not only help the company develop better and more complete and user-friendly The products provide more powerful services and more effective online media marketing. In addition, it can also provide favorable theoretical basis and conditions for the implementation of public opinion control for the government and related departments, so as to optimize social atmosphere.
	So, when I graduated from undergraduates, I chose to collect and analyze data based on social networks. This article proposes and implements a collection of documents based on the combination of Sina microblogging program interface and network data flow, and uses cookies to simulate the landing of Sina Weibo's web-site to achieve data capture and acquisition. And by monitoring the data packets transmitted during information exchange. Dynamically crawl page information. Using python language web crawlers and json database functions, parsing HTML XML to obtain user data information. Then the user node data is modeled by graph-based clustering social network algorithm and PageRank algorithm. Collect user's preferences and calculate the importance of their nodes. Analyze the social network relationships between users and users in Weibo. Analyze the importance of nodes through various algorithms and compare the accuracy of various algorithms. Degree, the influence evaluation model between them was implemented.
	The main problem of obtaining social network user data information discussed in this paper is that it solves the problem that microblogging is increasingly difficult to crawl large amounts of data in recent years and analyzes user behavior through data mining and obtains specific user influence. The force formula is based on the actual verification and error analysis.
	.}

\enkeyword{Social Network Data,  Acquisition Data,  Processing and Analysis,  Data Mining,  Graph Clustering}
...
\vbox{\centerline{\psfig{figure=01.eps,width=6cm}} \vskip1mm {\small
图1\quad 熔化率与比例、积分系数之间的关系
\\
Fig.\,1\quad Comparing melting rate with proportion and integral
parameters  of the Consarc controller }}}\end{center}
...
\begin{center}
{\centerline{\psfig{figure=02a.eps,width=6cm}}
{\footnotesize (a)控制参数1\\(a) Controller parameter 1}} \\
{\centerline{\psfig{figure=02b.eps,width=6cm}} {\footnotesize
